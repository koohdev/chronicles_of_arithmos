/*:
 * @target MZ
 * @plugindesc [V14] Multiplayer: Max Stability & Anti-Freeze.
 * @author Gemini AI
 *
 * @param Sync Interval
 * @desc Frames between updates. (Default: 2).
 * @type number
 * @default 2
 *
 ...
 * @help
 * ============================================================================
 * SIMPLE P2P: VERSION 14 (STABLE)
 * ============================================================================
 * * INSTRUCTIONS:
 * 1. Create Event: "RemotePlayer" (Trigger: Action Button).
 * 2. Host: SimpleP2P.host();
 * 3. Join: SimpleP2P.join("CODE");
 *
 * FEATURES:
 * - Join Battles (Click player to join).
 * - Party Merge (Host 1,2 + Client 1,2).
 * - Visual Sync (Icons, Sprites).
 * - Safety (Aborts battle if partner disconnects).
 * ============================================================================
 */

(() => {
    const params = PluginManager.parameters('SimpleP2P_V14');
    ...
    window.SimpleP2P = {
        ...
        myId: "",
        isConnected: false,
        originalParty: null, // Backup for Party Restore
        remoteData: {
            x: 0, y: 0, mapId: 0, 
        ...
        setupConnection: function(conn) {
            ...
            this.conn.on('close', () => {
                ...
                // If in battle, ABORT to prevent freeze
        ...
        // --- 2. UPDATE LOOP ---
        update: function() {
            // Safety: Don't run if game isn't ready
        ...
        handleData: function(data) {
            ...
            if (data.type === 'request_join') {
                if (SceneManager._scene instanceof Scene_Battle) {
                    ...
                    $gameMessage.add("Partner Joined Battle!");
                }
            }
        ...
        backupParty: function() {
            this.originalParty = $gameParty._actors.slice();
        },
        ...
        restoreParty: function() {
            if (this.originalParty) {
                ...
                $gamePlayer.refresh();
                // We don't need to refresh sprites here because battle is ending
            }
        },

        mergeParty: function(remoteActors) {
            if (!this.originalParty) this.backupParty();

            // Host keeps first 2
            const myActors = $gameParty._actors.slice(0, 2);
            // Combine
            const newParty = myActors.concat(remoteActors);
            $gameParty._actors = newParty;
            
            this.refreshBattleSprites();
        },
        ...
        joinBattleAsClient: function(troopId, hostActors) {
            ...
            const myActors = $gameParty._actors.slice(0, 2);
            $gameParty._actors = hostActors.concat(myActors);
            $gamePlayer.refresh();
        ...
        processGhostMovement: function() {
            ...
            if (dist > 10) {
                ...
                event.setMoveSpeed(dist > 4 ? 6 : (dist > 2 ? 5 : 4));
